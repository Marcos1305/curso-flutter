import 'package:animacoes_complexas/screens/home/widgets/animated_list.dart';
import 'package:animacoes_complexas/screens/home/widgets/fade_container.dart';
import 'package:animacoes_complexas/screens/home/widgets/home_top.dart';
import 'package:flutter/material.dart';

class StaggerAnimation extends StatelessWidget {
  final AnimationController controller;

  Animation<double> containerGrow;
  Animation<EdgeInsets> listSlidePosition;
  Animation<Color> fadeAnimation;

  StaggerAnimation({@required this.controller}) {
    containerGrow = CurvedAnimation(parent: controller, curve: Curves.ease);
    listSlidePosition = EdgeInsetsTween(
            begin: EdgeInsets.only(bottom: 0), end: EdgeInsets.only(bottom: 80))
        .animate(CurvedAnimation(
            parent: controller,
            curve: Interval(0.1325, .8, curve: Curves.ease)));
    fadeAnimation = ColorTween(
            begin: Color.fromRGBO(247, 64, 106, 1),
            end: Color.fromRGBO(247, 64, 106, 0))
        .animate(CurvedAnimation(parent: controller, curve: Curves.decelerate));
  }

  Widget _buildAnimation(BuildContext context, Widget child) {
    return Stack(
      children: <Widget>[
        ListView(
          padding: EdgeInsets.zero,
          children: <Widget>[
            HomeTop(containerGrow: containerGrow),
            AnimatedListView(
              listSlidePosition: listSlidePosition,
            )
          ],
        ),
        IgnorePointer(
          child: FadeContainer(
            fadeAnimation: fadeAnimation,
          ),
        )
      ],
    );
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: Container(
        child: AnimatedBuilder(
          animation: controller,
          builder: _buildAnimation,
        ),
      ),
    );
  }
}
